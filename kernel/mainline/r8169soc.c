 ...
 * which is Realtek's 4.9 BSP driver with the 4.9 -> 6.18 API changes done.
 * This copy only ever builds for the RTD129x, so the vendor's per-family
 * #ifdefs are kept and pinned to that one here rather than rewritten.
 */
#define CONFIG_ARCH_RTD129x 1

/*
 * Mainline has no RTD129x clock driver, so the "etn_sys"/"etn_250m" lookups
 * fail. Return NULL (not ERR_PTR) so the clk calls become no-ops, and gate
 * the clocks by hand instead -- see rtd129x_etn_clk().
 */
...
/*
 * The chip revision picks PHY calibration values. The BSP reads it through
 * drivers/soc/realtek/rtd129x/rtk_chip.c, which mainline does not have: the
 * cut is bits 17:16 of SB2 register 0x204. The previous copy of this driver
 * had a stub that always said A00, which would apply the A00-only AFE fix to
 * every chip.
 */
...
#define RTD129X_SB2_CHIP_REV	0x9801a204

static int rtd129x_chip_revision = -1;

/* First call is from probe, so later callers in atomic context never map. */
static int get_rtd_chip_revision(void)
{
	static const int cuts[] = {
		RTD_CHIP_A00, RTD_CHIP_A01, RTD_CHIP_B00, RTD_CHIP_B01,
	};
	void __iomem *reg;
	u32 val;

	if (rtd129x_chip_revision >= 0)
		return rtd129x_chip_revision;

	reg = ioremap(RTD129X_SB2_CHIP_REV, 4);
	if (!reg)
		return RTD_CHIP_A00;
	val = readl(reg);
	iounmap(reg);

	if ((val & 0xffff) || (val >> 16) >= ARRAY_SIZE(cuts)) {
		pr_warn("r8169soc: unknown chip revision 0x%08x, assuming B00\n",
			val);
		rtd129x_chip_revision = RTD_CHIP_B00;
	} else {
		rtd129x_chip_revision = cuts[val >> 16];
	}
	return rtd129x_chip_revision;
}
...
/*
 * ISO_CLK_EN bits 12:11 gate the ETN system and 250 MHz clocks, and
 * ISO_SOFT_RESET bits 10:9 hold the GPHY and GMAC in reset (active low).
 * With no clock driver the vendor's clk_prepare_enable() calls do nothing,
 * which only works if the bootloader happened to leave the clocks on --
 * true on the WD box this copy came from, not something to rely on here.
 */
#define ISO_SOFT_RESET		0x088
#define ISO_CLK_EN		0x08c
#define ISO_CLK_EN_ETN		(BIT(12) | BIT(11))
#define ISO_RSTN_ETN		(BIT(10) | BIT(9))

static void rtd129x_etn_clk(void __iomem *clkaddr, struct clk *sys,
			    struct clk *c250m, bool on)
{
	u32 tmp;
	...
	if (sys && c250m) {
		if (on) {
			...
			clk_prepare_enable(c250m);
		} else {
			clk_disable_unprepare(sys);
			clk_disable_unprepare(c250m);
		}
		return;
	}

	tmp = readl(clkaddr + ISO_CLK_EN);
	if (on)
		tmp |= ISO_CLK_EN_ETN;
	else
		tmp &= ~ISO_CLK_EN_ETN;
	writel(tmp, clkaddr + ISO_CLK_EN);
}

static bool rtd129x_etn_ready(void __iomem *clkaddr, struct clk *sys,
			      struct clk *c250m)
{
	if (sys && c250m)
		return __clk_is_enabled(sys) && __clk_is_enabled(c250m);

	return (readl(clkaddr + ISO_CLK_EN) & ISO_CLK_EN_ETN) == ISO_CLK_EN_ETN &&
	       (readl(clkaddr + ISO_SOFT_RESET) & ISO_RSTN_ETN) == ISO_RSTN_ETN;
}
...
rtl_init_one(struct platform_device *pdev)
{
	...
	#if defined(CONFIG_ARCH_RTD129x)
	dev_info(&pdev->dev, "chip revision %03x, ETN clocks %08x, resets %08x\n",
		 get_rtd_chip_revision(), readl(clkaddr + ISO_CLK_EN),
		 readl(clkaddr + ISO_SOFT_RESET));
	if (rtd129x_etn_ready(clkaddr, clk_etn_sys, clk_etn_250m)) {
		...
		/* ISO spec, clock enable bit for etn clock & etn 250MHz */
		rtd129x_etn_clk(clkaddr, clk_etn_sys, clk_etn_250m, true);

		/* 7. reg_0x9800708c[12:11] = 00 */
		/* ISO spec, clock enable bit for etn clock & etn 250MHz */
		rtd129x_etn_clk(clkaddr, clk_etn_sys, clk_etn_250m, false);
	...
	/*
	 * MAC0 holds whatever the bootloader left there. The BSP u-boot's is
	 * CONFIG_ETHADDR, 00:10:20:30:40:50 -- the same on every board -- and
	 * a MAC that was reset holds nothing usable. Take a random address
	 * instead: that marks it NET_ADDR_RANDOM, so udev's default
	 * MACAddressPolicy=persistent swaps in one derived from machine-id,
	 * which is unique per card and stays the same across boots.
	 */
	{
		static const u8 uboot_ethaddr[ETH_ALEN] = {
			0x00, 0x10, 0x20, 0x30, 0x40, 0x50,
		};

		if (!is_valid_ether_addr(ndev->dev_addr) ||
		    ether_addr_equal(ndev->dev_addr, uboot_ethaddr)) {
			dev_info(&pdev->dev, "no usable MAC address (%pM), using a random one\n",
				 ndev->dev_addr);
